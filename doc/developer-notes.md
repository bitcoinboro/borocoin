...
- [Development guidelines](#development-guidelines)
    - [General Borocoin Core](#general-litecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Borocoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Borocoin Core is a multi-threaded application, and deadlocks or other
...
Borocoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Borocoin Core, where everyone uses
...
pay attention to for reviewers of Borocoin Core code.

General Borocoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Borocoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Borocoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Borocoin Core so that they can be integrated
...
file descriptors it will cause problems with Borocoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Borocoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Borocoin Core, the wallet was always
