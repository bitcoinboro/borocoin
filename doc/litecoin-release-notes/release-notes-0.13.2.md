Borocoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Borocoin transactions now use validation using
...
Previous versions of Borocoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Borocoin Core was to raise the default minimum
relay fee.

Borocoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Borocoin Core 0.13.2 also introduces new default policy limits on the length and
...
Borocoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Borocoin Core will relay transactions with
...
entirely in Borocoin Core 0.13, and it is at this time undecided whether the
...
Borocoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Borocoin Core automatically creates a hidden service to listen on, without
manual configuration. Borocoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Borocoin Core is listening, and
...
Borocoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Borocoin
Core to use floating fees. By default, Borocoin Core will use floating
fees.
...
(default: `0.0002` BORO/kB).

At all times, Borocoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BORO.
Furthermore, Borocoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Borocoin Core code base has
...
for a new opcode in the Borocoin scripting system that in combination with
...
Borocoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Borocoin Core.
...
The two phases are distinct in the Borocoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Borocoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Borocoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Borocoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Borocoin addresses (both P2PKH addresses
  ...
  believed to be provided by Borocoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Borocoin's current ledger (called the UTXO set),
  ...
  note that Borocoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Borocoin users to individually opt-in to almost any change in the Borocoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Borocoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Borocoin
client to Borocoin Core 0.13.2 to signal for segwit support. When a
...
Since Borocoin Core 0.10.0, nodes have defaulted to only relaying and
